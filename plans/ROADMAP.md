...
Progreso global: `█████████░ 94% (15/16)` — **Fase 1 (MVP Finanzas) CERRADA** + **Fase 2: rebanadas 1 (eventos) + 2 (radar) + 3 (CII) + 4 (motor de convergencia, el último diferencial) + 5 (superficie UI de convergencia) CERRADAS**. Convergencia motor: `@www/core-signals`, ≥2 familias-de-dato disjuntas/72h/mag≥0.5, anti-doble-conteo por dataFamily, observaciones CII + markets exógeno, `convergence_signals` (migr 005) + dynamicScore, encadenado dentro del job cii, bloque en el briefing, calibración por-dimensión `conv-core-2`. Superficie UI (ADR-013): `GET /api/convergence` solo-lectura + capa **anillo** (distinto del CII, coexisten) + ConvergencePanel + 5ª pestaña + map-tie, camelCase. Verificado: suite **550/550** + global tsc + smoke curl + **browser E2E 19/19** (anillos render + map-tie + responsive). Siguiente: **completar dominios** (Finanzas FRED/EIA/OFAC, Educación rss+ONNX, Política ACLED/UCDP) o pulido (variantes, Tauri). Follow-ups convergencia: calibración fina DIMENSION_SCALE (GAP-2), markets change_pct null, completar COUNTRY_CENTROIDS (países panel-only).
